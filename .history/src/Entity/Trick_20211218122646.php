...
class Trick
{
     ...
     */
    public function getTaTricks(): Collection
    {
        return $this->trick_attachments;
    }

    public function addTaTrick(TrickAttachment $taTrick): self
    {
        if (!$this->trick_attachments->contains($taTrick)) {
            $this->trick_attachments[] = $taTrick;
            $taTrick->setTaTrick($this);
        }

        return $this;
    }

    public function removeTaTrick(TrickAttachment $taTrick): self
    {
        if ($this->trick_attachments->removeElement($taTrick)) {
            // set the owning side to null (unless already changed)
            if ($taTrick->getTaTrick() === $this) {
                $taTrick->setTaTrick(null);
            }
        }
