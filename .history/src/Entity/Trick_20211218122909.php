...
class Trick
{
    ...
    public function addTrickAttachment(TrickAttachment $taTrick): self
    {
        if (!$this->trick_attachments->contains($taTrick)) {
            $this->trick_attachments[] = $taTrick;
            $taTrick->setTrickAttachments($this);
        }

        return $this;
    }

    public function removeTrickAttachment(TrickAttachment $taTrick): self
    {
        if ($this->trick_attachments->removeElement($taTrick)) {
            // set the owning side to null (unless already changed)
            if ($taTrick->getTrickAttachment() === $this) {
                $taTrick->setTrickAttachment(null);
            }
        }
